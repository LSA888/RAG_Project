...
# --------------------------
# 静态页面路由：返回系统主页index.html（系统介绍 + 功能跳转入口）
...
async def get_index_page():
    ...
    html_abs_path = PROJECT_ROOT / "app/import_process/page/index.html"
    # 日志记录页面访问的文件路径，方便排查文件不存在问题
    logger.info(f"主页访问，文件绝对路径：{html_abs_path}")

    # 校验文件是否存在，不存在则抛出404异常
    if not os.path.exists(html_abs_path):
        logger.error(f"主页文件不存在，路径：{html_abs_path}")
