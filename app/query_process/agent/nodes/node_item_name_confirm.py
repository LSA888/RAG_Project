...
from app.clients.sqlite_history_utils import get_recent_messages, save_chat_message, update_message_item_names
...
def step_5_align_item_names(query_results: List[Dict]) -> Dict:
    ...
    for res in query_results:
        ...
        # 筛选
        # 阈值依据实测校准：同名/真实匹配 score≈0.90+，跨品类无关商品 score≈0.65
        # high>0.85 直接确认；mid 收紧到 0.75 作为"候选确认"，避免无关产品被误当候选弹出
        high = [m for m in matches if m.get("score", 0) > 0.85]
        mid = [m for m in matches if m.get("score", 0) >= 0.75]
